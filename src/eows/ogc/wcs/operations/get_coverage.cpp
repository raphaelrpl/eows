...
///        file.add_band(new eows::gdal::band(x*y*t, eows::gdal::band::INT16));
        ...
        properties.push_back(eows::gdal::property(index, eows::gdal::datatype::uint16));
//        file.add_band(new eows::gdal::band(x*y*t, eows::gdal::datatype::uint16));
//      else if (data_type == ::scidb::TID_INT8)
...
//  file.open();
  ...
  while(!cell_it->end())
  {
    ...
    for(std::size_t index = 0; index < attributes.size(); ++index)
    {
      ...
      if (data_type == ::scidb::TID_INT16)
        band->set_value(x, y, cell_it->get_uint16(attribute.getName()));
      else if (data_type == ::scidb::TID_INT8)
        band->set_value(x, y, cell_it->get_int8(attribute.getName()));
      else if (data_type == ::scidb::TID_UINT16)
        band->set_value(x, y, cell_it->get_uint16(attribute.getName()));
      else
  ...
  for(int i = 0; i < x; ++i)
  {
    for(int j = 0; j < y; ++j)
    {
      eows::gdal::band* band = file.get_band(0);
      band->get_value(i, j, tmp);
